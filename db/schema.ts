...
export const lotteryResults = pgTable(
  "lottery_results",
  {
    ...
    // a seat nobody can ever be shown (the page reads results by session
    // username, so an orphan row is invisible, not loud). Load `users.sql`
    // before the draw's SQL, and re-run the draw after any roster reload.
    //
